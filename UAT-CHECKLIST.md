...
| ultirecipe.recipe.count | `language: en` in config.yml; at least 2 custom recipes currently registered (run `ultirecipe.recipe.command-gate` first, then add at least one more recipe to `recipes.yml` and restart, since a fresh install ships zero recipes per `UltiKits/UltiRecipe#13`) Currently BLOCKED from execution entirely by `UltiKits/UltiRecipe#16` (any `recipes.yml` whose `recipes` map has at least one entry crashes module load with a `ClassCastException` — the shipped empty-map default (`recipes: {}`) loads fine) — this precondition cannot be established on the current build until #16 is fixed. | Run `/recipe count` | Chat/console line reads `Currently 2 recipes registered` (or the actual current count) in the exact `§7Currently §f<n> §7recipes registered` color/format template | server | |
| ultirecipe.recipe.list | `language: en` in config.yml; at least one custom recipe registered (see `ultirecipe.recipe.count`'s precondition) Currently BLOCKED from execution entirely by `UltiKits/UltiRecipe#16` (any `recipes.yml` whose `recipes` map has at least one entry crashes module load with a `ClassCastException` — the shipped empty-map default (`recipes: {}`) loads fine) — this precondition cannot be established on the current build until #16 is fixed. | Run `/recipe list` | Chat/console shows `=== Registered Recipes ===` (gold) followed by one `- <recipe-name>` line per registered recipe (gray dash, white name), then a `Total <n> recipes` summary line matching the count | server | |
...
| ID | Preconditions | Steps | Expected | Layer | Covers |
|---|---|---|---|---|---|
| ultirecipe.lifecycle.unload | `language: en` in config.yml; module loaded with at least one custom recipe registered (add one entry to `recipes.yml` and restart if none exists yet — one recipe is sufficient to exercise this hook). Currently BLOCKED from execution entirely by `UltiKits/UltiRecipe#16` (any `recipes.yml` whose `recipes` map has at least one entry crashes module load with a `ClassCastException` — the shipped empty-map default (`recipes: {}`) loads fine) — this precondition cannot be established on the current build until #16 is fixed (wave 1); the wave-0 session is expected to record this row `blocked` on that issue, and the wave-1 session re-runs it once #16 lands. | Unload the `UltiRecipe` module WITHOUT stopping or restarting the server (e.g. a plugin manager's live unload/disable command, such as PlugMan's `/plugman unload UltiRecipe`), then, with the module unloaded and the SAME server process still running, attempt to craft the registered recipe's shape in a crafting table. Do NOT use a full server stop, and do NOT use a stop-then-restart-without-the-module procedure, as the unload method for this row: a server restart rebuilds Bukkit's crafting-recipe registry from scratch regardless of whether `onUnregister()` ever ran, so the custom recipe would be absent either way and the row would false-pass without actually exercising the hook — only a live, in-process unload on the running server can distinguish "the hook removed it" from "the registry was rebuilt anyway." | The crafting result slot is empty for that shape (or whatever vanilla recipe happens to share the shape, if any) — the previously-registered custom recipe no longer exists in Bukkit's crafting registry, because `RecipeService#removeRecipes()` was called from `onUnregister()`, which the framework's own (now-final) `unregisterSelf()` invokes before its own command/listener cleanup for this module | server | |
| ultirecipe.lifecycle.reload | `language: en` in config.yml; module loaded with at least one custom recipe registered — same precondition as `ultirecipe.lifecycle.unload` above, and equally BLOCKED by `UltiKits/UltiRecipe#16` until wave 1; this row is expected to record `blocked` in the wave-0 session and be re-run once #16 lands. | Run `/ul reload UltiRecipe` through the framework's own reload command | Console shows, in this order: the framework's own per-module reload line `Module 'UltiRecipe' reloaded.`, then this module's own count line in the exact `[UltiTools] [UltiRecipe] Recipes reloaded, <n> recipes total` console format for the current recipe count — both bracketed prefixes are always present (`[UltiTools]` from Bukkit's own per-plugin logger, `[UltiRecipe]` added on top by `PluginLogger`; confirmed verbatim on a real Paper 1.21.11 server in `17-TRACER-LOAD-PROBE.md`) — the framework line appearing BEFORE this module's own line is exactly what proves `reloadSelf()`'s final template method ran its own steps (config reload, language refresh, `@ConditionalOnConfig` drift report) first, before invoking `onReload()`, which re-registers the module's recipes | server | |
