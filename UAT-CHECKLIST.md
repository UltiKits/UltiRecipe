...
- **Covers** back-references a GUI class excluded from the JaCoCo coverage gate; left blank when no such class
  applies. UltiRecipe has no GUI class excluded from the coverage gate (it ships no GUI page),
  so every
...
Both rows below exercise `UltiKits/UltiRecipe#11`'s lifecycle-hook migration:
...
| ID | Preconditions | Steps | Expected | Layer | Covers |
|---|---|---|---|---|---|
| ultirecipe.config.recipes-yml | `language: en` in config.yml (the log lines quoted below go through the language catalogue); fresh `config/recipes.yml` at its shipped default (`enabled: true`, `recipes: {}` — confirm the file truly ships an EMPTY `recipes` map, not the "golden_egg" example the README shows as a schema illustration; this module ships no `config/recipes.yml` resource and the dead seeder that claimed to write one was removed by `UltiKits/UltiRecipe#13`, with shipping an example tracked as `UltiKits/UltiRecipe#23`) | Load the file; confirm both keys (`enabled`, `recipes`) are present at their documented defaults; then add one recipe entry named `Custom_Sword` (mixed case) under `recipes` with a 3x3 `shape`, an `ingredients` map, and an `output` block (`material`, `amount`, `name`, `lore`), restart, and attempt to craft it, then run `/recipe list`; separately, and leaving the well-formed `Custom_Sword` entry in place, add a second recipe entry named `big_stack` and a third named `blank`, **each one a complete copy of `Custom_Sword`'s `shape` and `ingredients` blocks with a resolvable `output.material` (`DIAMOND`), differing from that copy in exactly one field**: `big_stack` sets `output.amount: 100` (outside the declared 1-64 `@Range`), `blank` sets `output.material: ""`. Then restart and run `/recipe list`. Both must be otherwise well-formed or this sub-step measures the wrong thing — `RecipeService#registerRecipe` refuses a missing `output`, `shape` or `ingredients` with the generic `Invalid recipe definition for: <name>` **before** it reaches the constraint check, so an entry written as bare as `blank:` + `output:` + `material: ""` produces that generic line instead, the Expected below never appears, and the row gets recorded `failed` against working code (`UltiKits/UltiRecipe#14`); separately, add a fourth recipe entry whose `shape` has only 2 rows and confirm it is skipped with a warning rather than registered; separately, add a fifth recipe entry named `bad name` (containing a space) and confirm it is skipped with a warning rather than registered or crashing the server; finally, leaving the well-formed `Custom_Sword` entry in place, add a sixth entry whose key is `broken` and whose whole value is the bare text `not a mapping` (no `output`, `shape` or `ingredients` beneath it), restart, and run `/recipe list`; lastly, delete the whole `ingredients:` block from the `Custom_Sword` entry, restart once more, run `/recipe list`, and attempt to craft the `Custom_Sword` shape | `enabled` and `recipes` present at their documented defaults before any change (`recipes` genuinely empty); after the change, the well-formed recipe crafts the configured output with its custom name/lore, and `/recipe list` shows it as `custom_sword` (lowercased), not `Custom_Sword` — the recipe's own name became part of a Bukkit `NamespacedKey`, which lowercases its input; the `big_stack` entry registers nothing and the server log shows exactly `[UltiTools] [UltiRecipe] Invalid output for recipe: big_stack - output.amount: value 100 is out of range [1, 64]` (NOT a stack of 100, and NOT a stack silently clamped to 64), while `/recipe list` still lists `custom_sword`; the `blank` entry likewise registers nothing and the server log shows exactly `[UltiTools] [UltiRecipe] Invalid output for recipe: blank - output.material: must not be empty` — that wording, naming `output.material`, is the assertion: the line an unrecognized material produces is the different `Invalid output material for recipe: <name>`, and before `UltiKits/UltiRecipe#14` both mistakes produced that second line (`UltiKits/UltiRecipe#14`); the 2-row-shape recipe registers zero output and the server log shows `[UltiTools] [UltiRecipe] Recipe shape must have exactly 3 rows for: <name>`; the `bad name` recipe registers zero output and the server log shows `[UltiTools] [UltiRecipe] Failed to register recipe: bad name - ...` (a `NamespacedKey` construction failure, caught by `initRecipes`'s try/catch); the `broken` entry registers nothing and the server log shows `[UltiTools] [UltiRecipe] Skipped recipe 'broken': expected a mapping, found the text 'not a mapping'`, while `/recipe list` still lists `custom_sword` and the module finishes loading (`UltiKits/UltiRecipe#16`); after the `ingredients:` block is deleted, the log shows `[UltiTools] [UltiRecipe] Invalid recipe definition for: Custom_Sword` and NO `Registered recipe: Custom_Sword` line, `/recipe list` does not list it, and the crafting table produces nothing for that shape (`UltiKits/UltiRecipe#16`) | server | |

## Language

| ID | Preconditions | Steps | Expected | Layer | Covers |
|---|---|---|---|---|---|
| ultirecipe.i18n.language | `language: en` in the framework's `plugins/UltiTools/config.yml`, set once for the session before the start; before the start, `plugins/UltiTools/pluginConfig/UltiRecipe/lang/` was moved into the session backup directory so this jar's catalogues are extracted fresh (an upgraded install never refreshes an already-extracted language file, UltiKits/UltiTools-Reborn#459); `enabled: true` in `config/recipes.yml` | From the server console run `/recipe help` | The console shows these four lines, and none of the lines this command printed contains Chinese text: `=== UltiRecipe Help ===`, `/recipe list - List all recipes`, `/recipe reload - Reload recipe config`, `/recipe count - Show recipe count` (colour codes rendered or stripped by the console, text otherwise exact) | server | |
