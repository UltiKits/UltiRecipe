...
- **即插即用** - 放入插件文件夹，自动生成配置文件（`enabled: true`、`recipes: {}`）。首次启动**不会**生成示例配方：
  下文的 `golden_egg`、`custom_sword` 等都是配方写法示例，需要自行写入 `recipes.yml`
  （「随插件附带一份示例配方」由 UltiKits/UltiRecipe#23 跟踪）
