...
| ultirecipe.config.recipes.recipes.\<name\>.shape | The 3x3 crafting-grid shape, exactly 3 strings each exactly 3 characters, space meaning "empty slot"; a shape with any row count other than 3 causes the whole recipe to be skipped with a warning ("Recipe shape must have exactly 3 rows for: `<name>`") — a per-row length other than 3 is NOT separately checked and would desynchronize the shape-to-ingredient character mapping silently | config | `config/recipes.yml: recipes.<name>.shape (list of exactly 3 strings)` | n/a | n/a | admin | detailed | RecipeService#registerRecipe |
